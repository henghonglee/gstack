...
**Tier 2 — global skills (automatic):** Also scan globally installed skills and user settings for the same patterns. Log in the report that global skill scanning was performed. This reads files outside the repo but is necessary for a complete security audit.

Run the same Grep patterns on globally installed skill files and check hooks in user settings.
...
**Remediation Roadmap:** For the top 5 findings, present in the report output with a recommended action for each:
1. Context: The vulnerability, its severity, exploitation scenario
2. RECOMMENDATION: [recommended action] because [reason]
3. Available actions:
   - Fix now — [specific code change, effort estimate]
   - Mitigate — [workaround that reduces risk]
   - Accept risk — [document why, set review date]
   - Defer to TODOS.md with security label
