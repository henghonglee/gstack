...
Auto-detect the deploy configuration from the codebase:

1. **Deploy trigger:** Scan for CI/CD config files (`.github/workflows/`, `Makefile`, `scripts/deploy*`, `Procfile`). Check `package.json` for deploy scripts. Infer the deploy method from what's found. Log: `[AUTO-DECISION] Deploy trigger: {detected method or "manual/unknown"}`

2. **Production URL:** Check for URLs in CI configs, environment files (`.env.example`, `.env.production`), README.md, and deployment configs. Also check for custom domain configs in platform files. Log: `[AUTO-DECISION] Production URL: {detected URL or "not detected — will need manual configuration"}`

3. **Health check:** Look for `/health`, `/api/status`, `/healthz` routes in the codebase. Check if the platform CLI is available. Log: `[AUTO-DECISION] Health check: {detected method}`

4. **Pre/post-merge hooks:** Check `package.json` scripts, `Makefile` targets, and CI configs for build/deploy steps.

If critical information cannot be detected (no production URL found, no deploy method identified), log what was found and what's missing: `[AUTO-DECISION] Partial detection — writing what was found to CLAUDE.md. Missing: {list}. Edit the Deploy Configuration section in CLAUDE.md to fill in the gaps.`
