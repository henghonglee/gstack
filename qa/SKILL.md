...
If the output is non-empty (working tree is dirty), auto-stash and proceed. Log: `[AUTO-DECISION] Working tree dirty — stashing uncommitted changes before QA. Will pop stash after QA completes.`

```bash
git stash push -m "gstack-qa: auto-stash before QA run"
```

Continue with setup. Pop the stash at the end of the QA workflow (after Phase 11).
...
11. **Clean working tree required.** If dirty, auto-stash before proceeding and pop stash after QA completes.
