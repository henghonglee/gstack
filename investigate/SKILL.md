...
1. **Collect symptoms:** Read the error messages, stack traces, and reproduction steps. If context is incomplete, proceed with available context and note assumptions made.
...
3. **3-strike rule:** If 3 hypotheses fail, **STOP and escalate in output:**
   ```
   3 hypotheses tested, none match. This may be an architectural issue
   rather than a simple bug. Escalating for human review.

   Hypotheses tested:
   1. [hypothesis 1] — disproved because [reason]
   2. [hypothesis 2] — disproved because [reason]
   3. [hypothesis 3] — disproved because [reason]

   Recommended next steps:
   - Escalate to someone who knows the system
   - Add logging to instrument the area and catch it next time
   ```
...
5. **If the fix touches >5 files:** Log the blast radius prominently in output and auto-decide:
   ```
   WARNING: This fix touches N files. Large blast radius for a bug fix.
   Decision: Proceeding — the root cause genuinely spans these files.
   [or] Decision: Splitting — fixing the critical path now, deferring the rest.
   ```
   Prefer splitting when possible. Only proceed with the full change if the root cause genuinely spans all files.
...
- **If fix touches >5 files →** log the blast radius prominently in output and prefer splitting.
- **Completion status:**
