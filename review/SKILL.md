...
If there are ASK items remaining, auto-decide each one based on the recommended fix:
...
Example output:
```
I auto-fixed 5 issues. 2 required individual auto-decisions:

1. [CRITICAL] app/models/post.rb:42 — Race condition in status transition
   [AUTO-DECISION] Applied fix: Added `WHERE status = 'draft'` to the UPDATE

2. [INFORMATIONAL] app/services/generator.rb:88 — LLM output not type-checked before DB write
   [AUTO-DECISION] Applied fix: Added JSON schema validation
```
...
2. **FALSE POSITIVE comments:** Auto-dismiss and log each one:
   - Log: `[AUTO-DISMISS] Greptile FP [file:line] — [reason it's wrong]`
   - Reply using the **False Positive reply template** from greptile-triage.md (include evidence + suggested re-rank), save to both per-project and global greptile-history.
...
Address the findings. Re-run `codex review` to verify.
