...
| `{{PREAMBLE}}` | `gen-skill-docs.ts` | Startup block: update check, session tracking, contributor mode, non-interactive decision format |
...
4. **Non-interactive mode** — all decisions are auto-decided with best judgment, choosing the RECOMMENDED option. Decisions are logged in output for user review.
